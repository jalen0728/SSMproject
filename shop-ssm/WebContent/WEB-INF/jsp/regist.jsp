...
<script>
	...
	function checkUsername(){
		...
		if(username_val==""||$.trim(username_val)==""){
			$("#span1").html("<font style='color:red'>用户名不能为空!!!</font>");
		}else{
			var url="${ pageContext.request.contextPath }/checkUsername";
			var args={"username":$.trim(username_val)};
			$.post(url,args,function(data){
				if(data=="yes"){
					$("#span1").html("<font style='color:red'>用户名已存在！！</font>");
				}else{
					$("#span1").html("<font style='color:green'>用户名可以使用！！</font>");
				}
			});
		}
	}
